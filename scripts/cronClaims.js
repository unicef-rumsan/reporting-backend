...
const { scripts, urlConfig } = require("./_common.js");
...
const claims = {
  async getLogsFromExplorer() {
    ...
    let abi = await scripts.getAbi("Rahat");
    try {
...
(async () => {
  await claims.getLogsFromExplorer();
  await claims.getModifiedDecodedLogs();
  await claims.updateMissingTransactionValue();
})();
